...
# 让用户输入红色像素的数量
try:
        red_pixels_count = int(input("请输入红色像素的数量（建议80-800之间）: "))
        # 限制红色像素的范围，避免过多或过少
        red_pixels_count = max(20, min(800, red_pixels_count))
except ValueError:
    print("输入无效，使用默认值700个红色像素")
    red_pixels_count = 700  # 使用默认值为700个红色像素
...
# 计算集中区域和分散区域的红色像素数量
# 92%在集中区域，8%随机分散
concentrated_count = int(red_pixels_count * 0.92)
dispersed_count = red_pixels_count - concentrated_count
...
for center_x, center_y in vein_centers:
    if 1 <= center_x < image_size - 1 and 1 <= center_y < image_size - 1:
        pixels[center_x, center_y] = random.choice(RED_COLORS)
        red_placed += 1
        if red_placed >= concentrated_count:
            break

# 然后从中心向外扩散形成矿脉（半径3像素，直径6像素）
while red_placed < concentrated_count and attempt < max_attempts:
    attempt += 1
            ...
            for dy in range(-2, 3):
                ...
                if euclidean_distance <= max_spread and 1 <= x < image_size - 1 and 1 <= y < image_size - 1:
                    ...
                    if random.random() < probability:
                                        # 只在非红色像素上填充，避免重复计算
                        if pixels[x, y] not in RED_COLORS:
                            pixels[x, y] = random.choice(RED_COLORS)
                            red_placed += 1
...
# 记录已经是黑色的像素位置，避免重复
# 收集所有非红色且非边缘颜色的像素位置作为候选
non_red_pixels = []
for x in range(1, image_size - 1):
    for y in range(1, image_size - 1):
        if pixels[x, y] not in RED_COLORS and pixels[x, y] != EDGE_GRAY:
            non_red_pixels.append((x, y))

# 随机选择分散的红色像素位置
if non_red_pixels and dispersed_count > 0:
    # 打乱候选列表
    random.shuffle(non_red_pixels)
    # 选择前dispersed_count个位置
    for x, y in non_red_pixels[:dispersed_count]:
        pixels[x, y] = random.choice(RED_COLORS)
        dispersed_placed += 1

# 确保总红色像素数量达到要求
total_red_placed = red_placed + dispersed_placed
if total_red_placed < red_pixels_count:
    # 如果还不够，在任何位置补充
    remaining = red_pixels_count - total_red_placed
